...
export default function EditOverview({
  ...
  const [mainDescription, setMainDescription] = useState(overview);
  const [benefits, setBenefits] = useState(keyBenefits.join(", "));
  const [mechanismText, setMechanismText] = useState(mechanism);
  const [reason, setReason] = useState("");
  const [sources, setSources] = useState("");

  const handleSubmit = (e: React.FormEvent) => {
    e.preventDefault();
    console.log({ mainDescription, benefits, mechanismText, reason, sources });
  ...
  const resetForm = () => {
    setMainDescription(overview);
    setBenefits(keyBenefits.join(", "));
    setMechanismText(mechanism);
    setReason("");
    setSources("");
  };
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <textarea
                  ...
                  rows={6}
                  className="w-full px-4 py-3 border border-gray-300 rounded-xl bg-white text-gray-900 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 cursor-text resize-none"
                />
              </div>
